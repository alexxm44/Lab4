"""
This program will allow a user three tries to guess the correct answer to the question
...
to see if the user input matches the answer. If so, then print "Correct!", then terminate the loop with a
break statement. 

If the user could not guess the correct answer witin the max_tries, then print "You have used up your allotment of guesses.",
...
ask
    ...
    loop three times
        ...
        check to see if user input is equal to answer
            if so then, print "Correct" then exit loop
    if not correct
